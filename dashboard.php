...
<div class="banner_bottom_in">
<?php
    $sql1 = mysqli_query($connect, "SELECT *, count(pendaftar_siswa.nik) as jml from pendaftar_siswa WHERE arsip = 'Tidak' AND keterangan = 'Belum Terverifikasi' ");
        $r = mysqli_fetch_array($sql1);
        $jml = $r['jml'];
    $sql1 = mysqli_query($connect, "SELECT *, count(pendaftar_siswa.nik) as jml_cowo from pendaftar_siswa WHERE jk = 'Laki - laki' AND arsip = 'Tidak' AND keterangan = 'Belum Terverifikasi'");
        $r = mysqli_fetch_array($sql1);
        $jmlcowo = $r['jml_cowo'];
    $sql1 = mysqli_query($connect, "SELECT *, count(pendaftar_siswa.nik) as jml_cewe from pendaftar_siswa WHERE jk = 'Perempuan' AND arsip = 'Tidak' AND keterangan = 'Belum Terverifikasi'");
        $r = mysqli_fetch_array($sql1);
        $jmlcewe = $r['jml_cewe'];
?>
<?php
    ...
    $todays_date = date("Y-m-d");
    if ($todays_date >= $start_date && $todays_date <= $end_date) { ?>
    <h1>Jumlah Pendaftar Hingga Saat Ini:<font style="color: red"> <?= $jml; ?></font></h1>
</div>
<br><br>
<?php } ?>
</div>
